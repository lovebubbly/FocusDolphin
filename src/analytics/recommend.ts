...
export function rankRecommendations(
  ...
  return domainStats
    ...
    .map((stats) => {
      const focusVisitRatio = focusHourRatio(stats.hourlyVisits, options.focusHours);
...
export function focusHourRatio(hourlyVisits: readonly number[], focusHours?: FocusHours): number {
  const totalVisits = hourlyVisits.reduce((sum, visits) => sum + Math.max(0, visits), 0);
  ...
  const focusVisits = hourlyVisits.reduce((sum, visits, hour) => {
    const hourMinute = hour * 60;
    return isMinuteInRange(hourMinute, startMinute, endMinute) ? sum + Math.max(0, visits) : sum;
  }, 0);
