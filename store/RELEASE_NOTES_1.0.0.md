# Focus Dolphin 1.0.0 Release Notes

Prepared by **OpenAI Codex (GPT-5)** for product owner **Choi Yunseong (최윤성)** and refreshed on **2026-07-12 KST**.

Focus Dolphin 1.0.0 is the first store candidate for the local-first Naver Whale and Chromium focus extension.

## Included

- Manual and scheduled focus sessions with soft, medium, and hard intensity.
- Direct 1-240 minute entry with adjacent five-minute step controls.
...
- A non-regressing dolphin companion with five stages, four moods, badges, streak protection, and post-session growth summaries.
...
- User-selectable Automatic, English, and Korean interfaces without resetting an active session.
- High-density dolphin rendering at every supported UI size and semantic motion for meaningful state changes.
...
Focus Dolphin has no developer backend, advertising, analytics SDK, remote AI, or remote code. Optional history analysis is user-invoked and local. Settings and pet progress may use browser-provider sync when enabled. See the public `PRIVACY.md` for the complete disclosure.
