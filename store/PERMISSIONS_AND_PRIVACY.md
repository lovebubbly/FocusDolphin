...
**Korean:** 사용자가 선택한 사이트 목록과 강도로 집중 세션을 운영하고, 완료한 집중을 고래 성장으로 보여주는 로컬 우선 브라우저 집중 도우미.

**English:** A local-first focus assistant that applies user-chosen site rules and turns completed sessions into whale growth.
...
- Raw history URLs and visit records are processed transiently and are not persisted by FocusWhale.
- Browser-provider sync may transmit settings, site lists, schedules, and pet state when the user has browser sync enabled; FocusWhale has no developer-operated server.
- Privacy-policy URL: <https://github.com/lovebubbly/FocusWhale/blob/main/PRIVACY.md>

The privacy policy contains the affirmative Chrome Web Store Limited Use statement. Local-only processing is still disclosed; the submission must not claim that FocusWhale handles no user data.
